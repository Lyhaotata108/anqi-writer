...
Batch flow:
    keywords -> variation brief -> brief-driven markdown -> quality guard -> preview -> CMS import
"""
...
from brief_article_writer import write_article_from_brief
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .card { background: #fff; border: 1px solid #d1d5db; border-radius: 14px; padding: 18px; box-shadow: 0 8px 24px rgba(17,24,39,.06); }
    h1 { margin: 0 0 6px; font-size: 24px; }
    .sub, .tiny { color: #6b7280; font-size: 13px; line-height: 1.5; }
    .grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-top: 18px; }
    .bucket { border: 1px solid #d1d5db; border-radius: 12px; padding: 14px; }
    .bucket-head { display: grid; grid-template-columns: 1fr 90px; gap: 10px; align-items: end; margin-bottom: 10px; }
    label { display: block; font-weight: 700; font-size: 13px; margin-bottom: 6px; }
    input, textarea { width: 100%; border: 1px solid #d1d5db; border-radius: 10px; padding: 10px 12px; font-size: 14px; outline: none; }
    input { height: 40px; }
    textarea { min-height: 150px; resize: vertical; font-family: Menlo, Consolas, monospace; line-height: 1.45; }
    button { height: 42px; border: 0; border-radius: 10px; padding: 0 14px; font-weight: 700; color: #fff; background: #111827; cursor: pointer; }
    ...
    pre, .markdown-box { width: 100%; min-height: 480px; border: 1px solid #d1d5db; border-radius: 12px; background: #fff; padding: 14px; white-space: pre-wrap; word-break: break-word; overflow: auto; font-family: Menlo, Consolas, monospace; font-size: 13px; line-height: 1.55; margin: 0; }
    .summary { min-height: 170px; max-height: 260px; margin-bottom: 12px; }
    .links { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
    .link { display: inline-block; padding: 6px 9px; border-radius: 8px; background: #eef2ff; color: #1d4ed8; text-decoration: none; font-size: 12px; font-weight: 700; }
    .link.warn { background: #fef3c7; color: #92400e; }
    .link.fail { background: #fee2e2; color: #991b1b; }
    @media (max-width: 1000px) { .grid, .main, .status-row { grid-template-columns: 1fr; } pre, .markdown-box { min-height: 320px; } }
  </style>
</head>
<body>
  <div class="wrap">
    <div class="card">
      ...
      <div class="sub">Category mapping: 1 减肥药 · 5 CBD · 9 Blood · brief-driven writer · quality guard · markdown preview · CMS import</div>
      <div class="grid">
        <div class="bucket">
          <div class="bucket-head"><div><label>Category 1 Name</label><input id="catName1" value="减肥药" /></div><div><label>ID</label><input id="catId1" value="1" /></div></div>
          <label>Keywords, one per line</label>
          <textarea id="keywords1" placeholder="paste data/keywords_weight_loss.to_generate.txt here"></textarea>
        </div>
        <div class="bucket">
          <div class="bucket-head"><div><label>Category 2 Name</label><input id="catName2" value="CBD" /></div><div><label>ID</label><input id="catId2" value="5" /></div></div>
          <label>Keywords, one per line</label>
          <textarea id="keywords2" placeholder="paste data/keywords_cbd.to_generate.txt here"></textarea>
        </div>
        <div class="bucket">
          <div class="bucket-head"><div><label>Category 3 Name</label><input id="catName3" value="Blood" /></div><div><label>ID</label><input id="catId3" value="9" /></div></div>
          <label>Keywords, one per line</label>
          <textarea id="keywords3" placeholder="paste data/keywords_blood.to_generate.txt here"></textarea>
        </div>
      </div>
      <div class="actions">
        ...
        <span class="tiny">建议先跑 100 个 cluster；每篇会保存 brief，并用 brief 直接生成正文。</span>
      </div>
...
function fillDemo() { document.getElementById('keywords1').value = 'berberine weight loss\nberberine belly\nberberine vs metformin'; document.getElementById('keywords2').value = 'cbd gummies for sleep\ncbd oil side effects'; document.getElementById('keywords3').value = 'cholesterol symptoms\nblood sugar after eating'; }
function clearAll() { ['keywords1','keywords2','keywords3'].forEach(id => document.getElementById(id).value = ''); }
function collectTasks() {
  const tasks = [];
  [1,2,3].forEach(i => {
    const category_id = document.getElementById('catId' + i).value.trim();
    const category_name = document.getElementById('catName' + i).value.trim();
    lines(document.getElementById('keywords' + i).value).forEach(keyword => tasks.push({keyword, category_id, category_name}));
  });
  return tasks;
}
function renderResultLinks(results) {
  ...
  currentResults.forEach((item, index) => {
    ...
    a.className = 'link';
    if (item.quality_passed === false) a.className += ' fail'; else if ((item.quality_score || 0) < 90) a.className += ' warn';
...
async function startBatchGeneration() {
  ...
  if (!tasks.length) { alert('Please paste at least one keyword.'); return; }
  ...
  setStatus('Queued', 5);
  const res = await fetch('/api/generate_batch', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({tasks})});
  const data = await res.json();
  if (!res.ok) { document.getElementById('startBtn').disabled = false; alert(data.error || 'Failed to start batch'); return; }
...
async function pollStatus() {
  ...
  const data = await res.json();
  setStatus((data.stage || data.status || 'Running') + ' · ' + (data.message || ''), data.percent || 0);
  ...
  if (data.status === 'done' || data.status === 'failed') {
    clearInterval(polling); polling = null; document.getElementById('startBtn').disabled = false;
...
async function publishToCms() {
  ...
  const res = await fetch('/api/publish', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({job_id: currentJobId})});
  const data = await res.json();
...
def _run_batch_generation(job_id: str, tasks: list[dict]) -> None:
    try:
        ...
        for index, task in enumerate(tasks, start=1):
            ...
            _append_log(job_id, "[Brief] " + f"lane={brief_json.get('lane')} · entity={brief_json.get('entity')} · intent={brief_json.get('intent')} · scene={brief_json.get('scene')} · brief={brief_txt_path.name}")

            _update_job(job_id, stage="Writing", percent=base + 5, message=f"Writing from brief: {keyword}")
            result = write_article_from_brief(
                keyword=keyword,
                brief=brief_json,
                output_root=OUTPUT_ROOT,
                category_id=category_id,
                keyword_id=None,
                stage1_1_path=brief_json_path,
                stage1_2_path=brief_txt_path,
            )
            ...
            if passed:
                ...
                _append_log(job_id, f"[Quality REVIEW] {keyword} · score={score} · issues={len(issues)}")
...
class BrowserUIHandler(BaseHTTPRequestHandler):
    server_version = "AnQiCMSBrowserUI/3.0"
